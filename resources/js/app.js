...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function updateGenerateButton() {
        const hasMedications =
            prescriptionBody.querySelectorAll("tr:not(:has(td[colspan]))")
                .length > 0;
    ...
    const fetchBranch = debounce(() => {
        ...
        fetch(
            `/branch?pharmacy=${encodeURIComponent(
                pharmacy
    ...
    const fetchStock = debounce(() => {
        ...
        fetch(
            `/stock?medication=${encodeURIComponent(
                name
            )}&branch_num=${encodeURIComponent(
                branchNum
            )}&branch_farm=${encodeURIComponent(branchFarm)}`,
            {
    ...
    btnAgregar.addEventListener("click", () => {
        ...
        const existingMeds = Array.from(
            prescriptionBody.querySelectorAll("tr td:nth-child(2)")
        )
        ...
        const rows = Array.from(prescriptionBody.querySelectorAll("tr"));
        rows.sort((a, b) => {
            const nameA = a
        ...
        row.querySelector(".btn-remove").addEventListener("click", () => {
            ...
            document
                .querySelectorAll(
                    `input.hidden-medication[data-index="${rowIndex}"]`
                )
                .forEach((input) => {
                    input.remove();
                });
            ...
            const remainingRows =
                prescriptionBody.querySelectorAll("tr").length;
